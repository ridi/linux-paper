...
#include <mach/iomux.h>//slr
//#define T63_BACK_LED 1
...
#if defined(CONFIG_MACH_RK3026_T63) && defined(T63_BACK_LED)
	struct timer_list butn_led_timer;
#endif
};
...
#if defined(CONFIG_MACH_RK3026_T63) && defined(T63_BACK_LED)
void light_up_butn_led(void)
{
	gpio_pull_updown(RK30_PIN2_PB5, GPIOPullDown);
		gpio_direction_output(RK30_PIN2_PB5, GPIO_HIGH);
	printk("open blue\n");
/*
	gpio_pull_updown(RK30_PIN2_PB6, GPIOPullDown);
			gpio_direction_output(RK30_PIN2_PB6, GPIO_LOW);
	printk("close green\n");
*/
}

static void button_led_timer(unsigned long _data)
{
	gpio_pull_updown(RK30_PIN2_PB5, GPIOPullDown);
...
static void keys_long_press_timer(unsigned long _data)
{
	...
	if(state) {	
		if(bdata->long_press_count != 0) {
			if(bdata->long_press_count % (LONG_PRESS_COUNT+HALF_ONE_SEC_COUNT) == 0){
...
#if defined(CONFIG_MACH_RK3026_T63) && defined(T63_BACK_LED)
				light_up_butn_led();
				mod_timer(&bdata->butn_led_timer,
...
#if defined(CONFIG_MACH_RK3026_T63) && defined(T63_BACK_LED)
			if (KEY_HOMEPAGE == button->code_long_press)
			{
				setup_timer(&bdata->butn_led_timer,
			    	button_led_timer, (unsigned long)bdata);
			}
#endif			
		}
		else if (button->code)
