...
int gt9157_detect_flag = 1;
static int first_i2c_test = 1;
//static struct regulator *ldo_tp = NULL;

EXPORT_SYMBOL_GPL(gt9157_detect_flag);
...
//slr copy
printk(KERN_ERR "[slr] goodix i2c test1\n");
...
printk(KERN_ERR "[slr] can not detect gt9157\n");
		gt9157_detect_flag = 0;
		return 0;
	}
...
#endif
	printk(KERN_ERR "[slr] goodix i2c test2\n");
    ret = gtp_i2c_test(client);
    if (ret < 0)
    {
        GTP_ERROR("I2C communication ERROR!");
...
//slr added
	...
	if (!ldo_tp) {
		printk("%s, tp power not available.\n", __func__);
		return -1;
	}
#endif	
    return 0;
...
{
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
printk(KERN_ERR "[slr] enter goodix_ts_remove\n");  

	if (0 == gt9157_detect_flag)
	{
#if GTP_ESD_PROTECT
	...
	//	kfree(ts);
		printk(KERN_ERR "[slr] goodix_ts_remove done\n");
		return 0;
	}
...
printk(KERN_ERR "GTP suspend------------------- system state : 0x%x\n", get_suspend_state());
	if (PM_SUSPEND_MEM == get_suspend_state())
	{
printk(KERN_ERR "GTP enter deep sleep\n");	
		#if 0
		gtp_irq_disable(ts);
		deep_sleep_flag = 1;
		#endif
#if 1//useful when use soware reset
		GTP_DEBUG_FUNC();
...
static void goodix_ts_late_resume(struct early_suspend *h)
{
    ...
    s8 ret = -1;
    ts = container_of(h, struct goodix_ts_data, early_suspend);
printk(KERN_ERR "GTP resume------------------- system state : 0x%x\n", get_suspend_state());    
	if (1 == deep_sleep_flag && PM_SUSPEND_ON == get_suspend_state())
	{
printk(KERN_ERR "GTP wakeup from deep sleep\n");
		#if 0
		gtp_irq_enable(ts);
		deep_sleep_flag = 0;
	    mdelay(20);
...
printk(KERN_ERR "[slr] enter goodix_ts_init <gt9157 start>\n");
    GTP_DEBUG_FUNC();   
	...
	if (0 == gt9157_detect_flag)
	{
		printk(KERN_ERR "[slr] not detect gt9157 so exit goodix_ts_init\n");
...
//slr modified for loading before the cyttsp4
