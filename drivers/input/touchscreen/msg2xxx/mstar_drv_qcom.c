...
#ifdef CONFIG_ENABLE_REGULATOR_POWER_ON
    g_ReguVdd = regulator_get(NULL, vdd_name);
#endif //CONFIG_ENABLE_REGULATOR_POWER_ON
...
//slr added
extern int gt9157_detect_flag;

static int __init touch_driver_init(void)
{
    int ret;

	if (1 == gt9157_detect_flag)
	{
		printk(KERN_ERR "[slr] detect gt9157 so exit mstar driver init\n");
		return 0;
	}
    /* register driver */
