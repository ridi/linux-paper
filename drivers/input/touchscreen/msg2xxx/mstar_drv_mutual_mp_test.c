...
static void _DrvMpTestItoShortTestSetPAD2GPO(u8 ItemID)
{
    ...
    DBG("ItemID %d, gpioNum %d",ItemID, gpioNum);
...
static void _DrvMpTestItoShortTestChangeANASetting(u8 ItemID)
{
...
static void _DrvMpTestItoShortTestMsg26xxm(u8 ItemID)
{
    int temp_deltaC[MAX_MUTUAL_NUM] = {0};
    int i;

    DBG("*** %s() ***\n", __func__);

    _DrvMpTestItoTestMcuStop();
    _DrvMpTestItoShortTestSetPAD2GPO(ItemID);
    _DrvMpTestItoShortTestChangeANASetting(ItemID);
    _DrvMpTestItoTestAnaSwitchToMutual();
    ...
    _DrvMpTestItoTestAnaSwReset();
    _DrvMpTestItoTestGetDeltaC(temp_deltaC);
    ...
    for (i = 0; i < MAX_MUTUAL_NUM ; i++)
    {
        if ((_gDeltaC[i] <= -(IIR_MAX)) || (temp_deltaC[i] <= -(IIR_MAX)) || (_gDeltaC[i] >= (IIR_MAX)) || (temp_deltaC[i] >= (IIR_MAX)))
        {
            _gDeltaC[i] = 0x7FFF;
        }
        else
        {
            _gDeltaC[i] = abs(_gDeltaC[i] - temp_deltaC[i]);
        }
        //DBG("ItemID%d, MUTUAL_NUM %d, _gDeltaC = %d\t", ItemID, i, _gDeltaC[i]);
    }
    DBG("\n");
}
...
static ItoTestResult_e _DrvMpTestItoShortTestMsg26xxmJudge(u8 ItemID)
{
