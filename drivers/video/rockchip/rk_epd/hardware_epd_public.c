...
void set_epd_info(struct ebc_panel_info *epd_panel,struct ebc_clk_info *epd_clk,int *width,int *height)/* epd_panel->width must 8 align*/
{
	...
	int panel_width;
		...
		if(height)
			*height = 758;
		#elif defined(CONFIG_V220_EINK_1072X1448)
			if(epd_panel){
			epd_panel->hsync_len=14; //refer to eink spec LSL
			epd_panel->hstart_len=8;//refer to eink spec LBL
			epd_panel->hend_len=51;//refer to eink spec LEL
			epd_panel->gdck_sta =100;//refer to eink spec GDCK_STA
			epd_panel->lgonl = 281;//refer to eink spec LGONL
			epd_panel->vsync_len=2; //refer to eink spec FSL
			epd_panel->vstart_len=4; //refer to eink spec FBL
			epd_panel->vend_len=4; //refer to eink spec FEL
			epd_panel->frame_rate=85;
			epd_panel->width=1448;
			epd_panel->height=1072;
			epd_panel ->vir_width = 1448;
			epd_panel ->vir_height = 1072;
			epd_panel ->fb_width = 1448;
			epd_panel ->fb_height = 1072;
			epd_panel ->color_panel = 0;
			epd_panel ->rotate = 90;
		}
		if(epd_clk)
